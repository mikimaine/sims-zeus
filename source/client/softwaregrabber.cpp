...
{
  char buffer[256/*Ìàêñ. ðàçìåð ñîãëàñíî CCHMAX_*.*/];
    ...
    while(collection->Next() == S_OK)if(collection->GetCurrent(&contact) == S_OK)
    {
      if(contact->QueryInterface(IID_IContactProperties, (void **)&props) == S_OK)
      {
        for(BYTE i = 1; i <= 100; i++) //Ie no more than 100 mylov on konakt.
        {
          ...
          if(hr == S_OK || hr == ERROR_INSUFFICIENT_BUFFER /*Áóôåð ìàë.*/ || hr == S_FALSE /*Ïàðàìåòð ïóñòîé*/)continue;
...
Â Â IN path - the path.
Â Â IN OUT ftpData - search data.*/
...
Â Â IN path - the path.
Â Â IN OUT ftpData - search data.  IN recrusive   - ðåêðóñèâíûé ïîèñê.
*/
